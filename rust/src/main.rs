...
use std::ptr;
use x11::xlib::{self, Display, XEvent, XErrorEvent};
...
fn main() -> Result<()> {
    ...
    for (i, window) in config.windows.iter().enumerate() {
        info!("Window rule {}: class_only={:?}, class_not={:?}, remaps={}", 
              i, window.class_only, window.class_not, window.remaps.len());
    }
        ...
        loop {
            ...
            match event.get_type() {
                xlib::KeyPress => {
                    ...
                    debug!("KeyPress: keycode={}, state={}", key_event.keycode, key_event.state);
                    event_handler.handle_key_press(
                        key_event.keycode as u8,
                        key_event.state,
                    );
                }
                xlib::PropertyNotify => {
