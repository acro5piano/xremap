...
use x11::xlib::{self, Display, Window, KeyCode};
...
impl EventHandler {
    ...
    pub fn handle_key_press(&mut self, keycode: KeyCode, state: u32) {
        ...
        debug!("Handling key press: keycode={}, state={:#x}, filtered_state={:#x}", 
               keycode, state, filtered_state);
        
        if let Some(handler) = self.key_handlers.get(&key_press) {
            info!("Found handler for keycode={}, state={:#x}, executing remap", keycode, filtered_state);
            handler();
        } else {
            debug!("No handler found for keycode={}, state={:#x}", keycode, filtered_state);
            debug!("Available handlers: {:?}", self.key_handlers.keys().collect::<Vec<_>>());
        }
    }
    ...
    fn update_key_mappings(&mut self) {
        ...
        let window_class = active_window
            .and_then(|w| self.window_manager.get_window_class(w));
        
        info!("Active window: {:?}, class: {:?}", active_window, window_class);
    ...
    fn register_remap(&mut self, remap: Remap, target_window: Option<Window>) {
        if let Some((from_keysym, from_mods)) = self.key_mapper.parse_key(&remap.from) {
            ...
            debug!("Registering remap: '{}' (keysym={:#x}, mods={:#x}) -> keycode={}, to={:?}", 
                   remap.from, from_keysym, from_mods, keycode, remap.to);
            
            if keycode == 0 {
                warn!("Failed to get keycode for keysym {:#x} (key '{}')", from_keysym, remap.from);
                return;
            }
            ...
            debug!("Successfully registered handler for keycode={}, mods={:#x}", keycode, from_mods);
        } else {
    ...
    fn grab_keys(&self) {
        unsafe {
            ...
            for key_press in &self.grabbed_keys {
                debug!("Grabbing key: keycode={}, modifiers={:#x}", 
                ...
                if grab_result != 0 {
                    ...
                    debug!("Successfully grabbed key: keycode={}, modifiers={:#x}", 
                          key_press.keycode, key_press.modifiers);
                }
